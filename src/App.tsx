...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<'home' | 'login' | 'admin' | 'client'>('home');
  ...
  const handleLogin = (loggedInUser: User) => {
    ...
    setView(loggedInUser.role === 'admin' ? 'admin' : 'client');
  };
    ...
    <main className="min-h-screen relative selection:bg-stone-200">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="font-serif text-2xl md:text-3xl tracking-wide cursor-pointer z-50" onClick={() => scrollToSection('home')}>
            HAMZA SOULI
          </div>
      ...
      >
        {navLinks.map(link => (
          <button key={link.id} onClick={() => scrollToSection(link.id)} className="font-serif text-4xl text-stone-900 hover:text-stone-600">
            {link.name}
          </button>
        ))}
