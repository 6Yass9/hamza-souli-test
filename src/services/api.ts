...
export const api = {
  ...
  loginStaffOrAdmin: async (email: string, password: string): Promise<User | null> => {
    ...
    if (error) {
      console.error('loginStaffOrAdmin error', error);
      return null;
    }
    if (!data) return null;
  ...
  loginClient: async (code: string): Promise<User | null> => {
    const { data, error } = await supabase
      .from('users')
      .select('*')
      .eq('login_code', code)
  ...
  createClient: async (
    ...
    if (!cleanName || !cleanLoginCode) {
      throw new Error('Name and login code are required');
    }
    ...
    if (existing) {
      throw new Error('Login code already in use');
    }
  ...
  // ✅ NEW: fetch only appointments assigned to a staff member
  ...
  createAppointment: async (date: string, name: string, email: string) => {
    const payload = {
      ...
      // staff_id can be assigned later by admin
    };
  ...
  updateAppointment: async (id: string, updates: Partial<Appointment>): Promise<Appointment | null> => {
    ...
    // ✅ NEW: allow admin to assign staff
